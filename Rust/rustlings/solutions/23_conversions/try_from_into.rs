...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let (red, green, blue) = (convert(tuple.0)?, convert(tuple.1)?, convert(tuple.2)?);

        Ok(Color { red, green, blue })
    }
}

fn convert<T, U: TryFrom<T>>(color: T) -> Result<U, IntoColorError> {
    U::try_from(color).map_err(|_| IntoColorError::IntConversion)
}
